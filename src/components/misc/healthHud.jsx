...
// importa el componente UmbrellaCard que creamos antes
import UmbrellaIdCard from "./UmbrellaIdCard";

export default function HudWithModal() {
  const [status, setStatus] = useState("danger"); // estado inicial
  ...
  const barColors = {
    fine: "from-green-800 via-green-600 to-green-300",
    caution: "from-yellow-500 via-yellow-400 to-yellow-200",
    danger: "from-red-600 via-red-500 to-red-300",
  };
  ...
  const handleClick = () => {
    if (status === "danger") {
      ...
      setStatus("fine");
      setShowModal(true); // abrimos la Umbrella ID Card
    } else {
      setStatus("danger"); // reinicia ciclo
    }
  };
  ...
  return (
    <>
      ...
      >
        ...
        {/* Noise */}
        <div
          className="absolute inset-0 opacity-10 pointer-events-none mix-blend-soft-light animate-[jitter_260ms_steps(2,end)_infinite]"
          style={{
        ...
        {/* Barra de vida con gradiente dinámico */}
        <div
          className={`absolute left-[26%] right-4 bottom-3 h-1 rounded-sm bg-gradient-to-r ${barColors[status]} opacity-80`}
        />
        {/* Texto de estado */}
        <div
          className={`absolute right-4 bottom-4 text-xl font-mono tracking-wider animate-[pulse_1.5s_ease-in-out_infinite, flick_5.5s_steps(1,end)_infinite] ${colorClasses[status]}`}
        >
        ...
        <style jsx>{`
          ...
          @keyframes pulse {
            0%,
            100% {
              text-shadow: 0 0 4px rgba(65, 255, 106, 0.35),
                0 0 14px rgba(65, 255, 106, 0.25);
            }
            50% {
              text-shadow: 0 0 10px rgba(65, 255, 106, 0.7),
                0 0 26px rgba(65, 255, 106, 0.5);
            }
          }
          @keyframes jitter {
            50% {
      ...
      {/* Modal con Umbrella Card */}
      {showModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/70 z-[9999]">
          <UmbrellaIdCard onClose={() => setShowModal(false)} />
        </div>
      )}
    </>
  );
